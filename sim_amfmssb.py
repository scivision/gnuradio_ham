...
from gnuradio import audio, analog, blocks, digital, filter, gr, qtgui
...
#%% transmitter
    ...
    self._tonelevel_dB_win = RangeWidget(self._tonelevel_dB_range, self.set_tonelevel_dB, "Mod. Level [dB]", "counter_slider", float)
    self.top_layout.addWidget(self._tonelevel_dB_win)

    self.Ptx_dBm = 30.
    self._Ptx_dBm_range = Range(-30,60,1,30,200)
    self._Ptx_dBm_win = RangeWidget(self._Ptx_dBm_range,self.set_Ptx_dBm, "Transmit Power [dBm]","counter_slider",float)
...
#%% receiver
    ...
    self.frx = 50e3
    self._frx_range = Range(40e3, 60e3, .1e3, 50e3, 200)
    self._frx_win = RangeWidget(self._frx_range, self.set_frx, "RX freq. [Hz]", "counter_slider", float)
...
#%% transmitter
    self.mod_upsample = filter.rational_resampler_ccc(
            interpolation=self.samp_rate//self.fs_audio,
            decimation=1,
            taps=None,
            fractional_bw=None,)

    self.mod_source = analog.sig_source_f(self.fs_audio, analog.GR_SIN_WAVE,
                                          1000, 10**(self.tonelevel_dB/20), 0)
...
#%% receiver
    ...
    self.rx_downsample = filter.rational_resampler_ccc(
            interpolation=1,
            decimation=self.samp_rate//self.fs_audio,
            taps=None,
            fractional_bw=None,)

    if modtype=='am':
        self.rxif_filter = filter.fir_filter_ccc(1, firdes.low_pass(
            	1, self.fs_audio, self.bwrx/2, 200, firdes.WIN_HAMMING, 6.76))
        ...
        self.demod = analog.nbfm_rx(
                        	audio_rate=self.fs_audio,
                        	quad_rate=self.fs_audio,
                        	tau=75e-6,
                        	max_dev=self.fmbw,
                             )
    elif modtype=='ssb':
        self.rxif_filter = filter.fir_filter_ccc(1, firdes.band_pass(
    ...
    self.audio_bpf = filter.fir_filter_fff(1, firdes.band_pass(
        	1, self.fs_audio, 300, 3400, 200, firdes.WIN_HAMMING, 6.76))
...
#%% Connect transmitter
        ...
        self.connect(self.tx_mixer,
                     self.transmit_amplifier,
                     self.pathloss)
...
#%% connect receiver
        ...
        if modtype=='am':
            ...
            self.connect(self.channeladder,
                         self.rxif_filter,
                         (self.rx_mixer,1),)
            self.connect(self.rx_mixer,
                         self.rx_downsample,
...
#%% plots
        ...
        if 0:
            scope_rxif_filter = scope("RX IF filtered",self.samp_rate,self.top_layout,2e-6,complex)
            self.connect(self.rxif_filter,scope_rxif_filter)

        if 1:

            if modtype in ('am','ssb'):
                specan_rxif = specan("RX IF",self.samp_rate,self.top_layout,complex,False,(-180,-100))
                self.connect(self.rxif_filter,specan_rxif)
            else:
                specan_rxif = specan("RX IF",self.fs_audio,self.top_layout,complex,False,(-180,-100))
        ...
        if 1:
            specan_rxaudio = specan("RX audio",self.fs_audio,self.top_layout,float,False,(-100,0))
